...
int main(void) {
  ...
  //check(linear, 0, 0, 0, "1.testing for x - 2.5");
  check(linear, 0, 1024, 2, "2.testing for x - 2.5");
  check(linear, 0, 3, 2, "3.testing for x - 2.5");
  check(linear, 1, 3, 2, "4.testing for x - 2.5");
  check(linear, 2, 3, 2, "5.testing for x - 2.5");
  check(linear, 1, 2, 1, "6.testing for x - 2.5");
  //all positive
  check(linear, 10, 1024, 10, "7.testing for x - 2.5");

  delete linear;

  Function<int, int> * segFunc = new SegFunction();
  check(segFunc, -1, 1, -1, "8.testing for seg func");
  check(segFunc, 0, 1, 0, "9.testing for seg func");
  delete segFunc;
